...
def scrape_data(start_date, end_date):
    ...
    desired_data = []   
    for area in list(area_info.keys())[5:]:
        ...
        for (colliery_name, colliery_id) in collieries:
            ...
            print("Scraping for Colliery {} ({}) in {} ({})".format(
                colliery_name, colliery_id, area_name, area_id))
            r = requests.post(url, data=params)

            scrape_rows(r.content, area_id, colliery_id,
                        area_name, colliery_name)
...
def process(temp):
    ...
    if 'Sales Order Expiry Date' in temp:
        temp['Sales Order Expiry Date'] = str_to_dt(
            temp['Sales Order Expiry Date'])
...
def save_in_pickle():
    global all_area_data
    with open("scraped_data2.pickle", "wb") as file:
...
def scrape_past_dates(start_date="04-01-2019", end_date="19-01-2019"):
