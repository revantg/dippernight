...
with open("sales_orders_data2.pickle", "rb") as file:
...
def process(json_data):
    if 'Lifting Date' in json_data:
        json_data['Lifting Date'] = datetime.datetime.strptime(json_data['Lifting Date'], "%d-%m-%Y")
    return json_data
...
def save_data(sales_orders_data):
    ...
    with open("sales_orders_data2.pickle", "wb") as file:
